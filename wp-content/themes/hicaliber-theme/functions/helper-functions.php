        ...
        foreach( $meta as $m ) {
            ...
            if( !empty( $m['type'] ) ) {
                ...
                */
                ...
                if( $m['type'] == 'flag' ) { 
                    $joins[] = "LEFT JOIN {$wp->prefix}postmeta AS {$m['key']} ON {$m['key']}.post_id = posts.ID AND {$m['key']}.meta_key = '{$m['key']}'";
                    $wheres[] = "AND ({$m['key']}.meta_value IS NULL OR {$m['key']}.meta_value = 0 )"; 
                }
                
                
                 if( $m['type'] == 'flag_2' ) { 
                    $joins[] = "LEFT JOIN {$wp->prefix}postmeta AS {$m['key']} ON {$m['key']}.post_id = posts.ID AND {$m['key']}.meta_key = '{$m['key']}'";
                    $joins[] = "LEFT JOIN {$wp->prefix}postmeta AS {$m['key_2']} ON {$m['key_2']}.post_id = posts.ID AND {$m['key_2']}.meta_key = '{$m['key_2']}'";
                    $wheres[] = "AND ( ( {$m['key']}.meta_value IS NULL OR {$m['key']}.meta_value = 0 ) AND ( {$m['key_2']}.meta_value IS NULL OR {$m['key_2']}.meta_value = 0 ) )"; 
                } 
                ...
                if( $m['type'] == 'date' ) { 
                    ...
                    if( $m['compare'] == 'upcoming' ) {
                     ...
                     if( $m['compare'] == 'current' ) {
                        ...
                        $wheres[] = "AND ( DATE_FORMAT({$start_date_mata_value}, '%%Y-%%m-%%d 00:00:00') <= NOW() AND DATE_FORMAT($end_date_mata_value, '%%Y-%%m-%%d 23:59:59') >= NOW() )";
                    }
                ...
                if( !empty( $m['compare'] ) ) {
                    ...
                    if( $m['compare'] == 'LIKE%' ) {
                        $wheres[] = "AND {$m['key']}.meta_value LIKE '%%{$m['value']}%%'";  
                    } else {
...
public static function qry( $args = [] ){
    ...
    // if ($args['post_type'] == 'ac-promotions') {
    //     $order = "ORDER BY promotion_period_start_date.meta_value ASC";
    // } else {
    //     $order = "ORDER BY posts.post_date DESC";
    // }
